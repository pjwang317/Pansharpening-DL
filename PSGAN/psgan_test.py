...
parser.add_argument("--test_tfrecord", help="filename of test_tfrecord", default="C:/Users/USER-1/Documents/Peijuan/article/fromHocam/psgan/PSGan-master/data/psgan/test_sim_WV3.tfrecords")
...
def load_examples():
    if a.mode == 'train':
        ...
        #filename_queue = tf.compat.v1.train.string_input_producer([a.train_tfrecord])
    elif a.mode =='test':
        filename_queue = tf.train.string_input_producer([a.test_tfrecord])
        #filename_queue = tf.compat.v1.train.string_input_producer([a.test_tfrecord])
    ...
    for i, in_path in enumerate(fetches["imnames"]):
        ...
        for kind in ["inputs1","inputs2", "outputs","targets"]:
            ...
            if kind is not "inputs2":
                ...
                array2raster(out_path, [0, 0], 200, 200, contents.reshape((200,200)), 1)
