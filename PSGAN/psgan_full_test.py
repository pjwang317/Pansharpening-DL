...
parser.add_argument("--test_tfrecord", help="filename of test_tfrecord", default="C:/Users/USER-1/Documents/Peijuan/article/fromHocam/psgan/PSGan-master/data/psgan/test_real_WV3.tfrecords")
...
parser.add_argument("--test_count", type=int, default=100, help="number of test data")
a=parser.parse_args()
...
def create_model(inputs1, inputs2, targets):
    def create_discriminator(discrim_inputs, discrim_targets):
        print(discrim_inputs.get_shape)
        n_layers = 3
        layers = []
    ...
    with tf.variable_scope("generator") as scope:
        out_channels = int(inputs1.get_shape()[-1])
        outputs = create_generator(inputs1, inputs2, out_channels)

    with tf.name_scope("real_discriminator"):
        with tf.variable_scope("discriminator"):
            predict_real = create_discriminator(inputs1, outputs)
    ...
    with tf.name_scope("generator_loss"):
        ...
        gen_loss_L1 = tf.reduce_mean(tf.abs(outputs - outputs))
...
def save_images(fetches, step=None):
    ...
    for i, in_path in enumerate(fetches["imnames"]):
        ...
        for kind in ["inputs1","inputs2", "outputs","targets"]:
            ...
            if kind is "targets":
                array2raster(out_path, [0,0], 200, 200, contents.transpose(2,0,1), 4)
            elif kind is "outputs":
                array2raster(out_path, [0, 0], 800, 800, contents.reshape(800,800,4).transpose(2,0,1), 4)
            elif kind is "inputs1":
                array2raster(out_path, [0, 0], 800, 800, contents.reshape(800,800,4).transpose(2,0,1), 4)
            else:
                array2raster(out_path, [0, 0], 800, 800, contents.reshape(800,800), 1)
...
def main():
    ...
    #model = create_model_test(examples.inputs1, examples.inputs2)
