...
weightb=loadmat('C:/Tools/MATLAB/mydoc/networks/params/net64_05.mat')
...
def getdata(data1,data2):
    ...
    input_data=np.empty((size,60,60,5))
    ...
    for i in range(0,size):
        ...
        MS_ref[i,:,:,:]=Y_
        band=np.array([Y_11.T,Y_22.T,Y_33.T,Y_44.T,X_1.T])
...
with tf.name_scope('inputs'):
    ...
    tf_x=tf.placeholder(tf.float32, [BATCH_SIZE,76,76,5],name='PAN_input')
    I_X=tf.reshape(tf_x,[-1,76,76,5])
...
with tf.name_scope('weights'):
    ...
    weights = {
		    ...
		    'w4': tf.Variable(tf.random_normal([11,11,5,60],stddev=1e-3), name='w4'),
...
#### resnet laybers
    ...
    conv5_2=tf.nn.relu(tf.nn.conv2d(conv4,weights['w6'],
                                 strides=[1,1,1,1],
                                 padding='SAME')+biases['b6'],name='conv5_2')
    conv5_3=tf.nn.relu(tf.nn.conv2d(conv4,weights['w7'],
                                 strides=[1,1,1,1],
                                 padding='SAME')+biases['b7'],name='conv5_3')
    conv5 = tf.concat([conv5_1,conv5_2,conv5_3],axis=3)+conv4
           ####conv5 = tf.layers.batch_normalization(conv5_4,momentum=0.4)+conv4
		   
    conv6 = tf.nn.relu(tf.nn.conv2d(conv5,weights['w8'],
                                 strides=[1,1,1,1],
                                 padding='VALID')+biases['b8'],name='conv6')
		   
    conv7_1 =tf.nn.relu(tf.nn.conv2d(conv6,weights['w9'],
                                 strides=[1,1,1,1],
                                 padding='SAME')+biases['b9'],name='conv7_1')
    conv7_2 =tf.nn.relu(tf.nn.conv2d(conv6,weights['w10'],
                                 strides=[1,1,1,1],
                                 padding='SAME')+biases['b10'],name='conv7_2')
    conv7_3 =tf.nn.relu(tf.nn.conv2d(conv6,weights['w11'],
                                 strides=[1,1,1,1],
                                 padding='SAME')+biases['b11'],name='conv7_3')
...
with tf.Session() as sess:
    ...
    for step in range(300):
        ...
        if (step+1) % 50 ==0:
            ...
            save_path=saver.save(sess,"C:/Tools/Python/Python35/RemoteSensingEx/myCodes/training/newdata/parameter/MSDCNN_noindices_1203.ckpt")
            ...
            savemat('%s' % (step+1) + '_resnet_no_indices_L1_1203.mat',{'W1':W1,'W2':W2,'W3':W3,'W4':W4,'W5':W5,'W6':W6,
